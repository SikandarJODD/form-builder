<script lang="ts">
	...
	const fetchGitHubData = async () => {
		try {
			let repoResponse = await fetch('https://api.github.com/repos/sikandarjodd/form-builder');
...
<div class="bg-background">
	<div class="relative isolate px-6 pt-0 md:pt-20 lg:px-8">
		...
		/>
		<div class="mx-auto py-24 sm:py-48 md:max-w-2xl lg:py-32">
		...
		<div class="space-y-3">
			...
			<div class="flex flex-col gap-3 md:flex-row md:gap-5">
				<div class={'z-10 flex -space-x-12 rtl:space-x-reverse'}>
					<a href="https://github.com/sponsors/SikandarJODD" target="_blank" class="group">
						<Button
							class="flex h-9 w-38 justify-start rounded-full border-2 border-white shadow-xs sm:h-10 dark:border-gray-800"
						>
							<HeartIcon />
							Sponsor
						</Button>
					...
					<a
						...
						target="_blank"
						class="group flex h-9 w-auto cursor-pointer items-center space-x-1 rounded-full border-2 border-white bg-muted px-3 whitespace-pre shadow-xs hover:shadow-lg sm:h-10"
					>
						<p class="hidden text-sm font-medium text-primary md:flex">Star Project on GitHub</p>
						<div class="flex items-center rounded-full px-2 py-1 text-center text-sm font-medium">
							<svg
...
{#snippet heroTitle()}
	<div class="flex w-full max-w-2xl flex-col space-y-4 overflow-hidden pt-2">
		<Motion.h1
			class="text-center text-4xl leading-tight font-semibold text-foreground sm:text-5xl md:text-6xl"
		...
		>
			...
			<a
				...
				target="_blank"
				class="text-primary/90 underline underline-offset-2"
			>
				Shadcn Svelte
			</a>,{' '}
			<a
...
{#snippet heroCTA()}
	<Motion.div
		class="mx-auto mt-3 flex w-full flex-col items-center justify-center space-y-4 sm:mt-6 sm:max-w-2xl sm:flex-row sm:space-y-0 sm:space-x-4"
		initial={{ opacity: 0, y: 20 }}
		animate={{ opacity: 1, y: 0 }}
	...
	>
		<Button
			...
			variant="secondary"
			href="/playground"
			class="w-full rounded-full sm:w-fit sm:rounded-lg">Go to Playground</Button
		>
	</Motion.div>
{/snippet}

<script>
	...
	import SubSoln4 from './SubSoln4.svelte';
	...
	let features = [
		{
			...
			class: 'hover:bg-red-500/10 transition-all duration-500 ease-out',
			content: SubSoln1
		},
		{
			...
			class: 'order-3 xl:order-0 hover:bg-blue-500/10 transition-all duration-500 ease-out',
			content: SubSoln2
		},
		{
			...
			class: 'md:row-span-2 hover:bg-orange-500/10 transition-all duration-500 ease-out',
			content: SubSoln3
		},
		{
...
>
	<div
		class="mx-auto mt-16 grid max-w-sm grid-cols-1 gap-6 text-gray-500 md:max-w-3xl md:grid-cols-2 md:grid-rows-3 xl:max-w-6xl xl:auto-rows-fr xl:grid-cols-3 xl:grid-rows-2"
	>
		{#each features as feature, index}
			{@const Component = feature.content}
			<div
				class={cn(
			...
			>
				...
				<Component />
				<div
					class="pointer-events-none absolute bottom-0 left-0 h-32 w-full bg-linear-to-t from-neutral-50 dark:from-neutral-900"
				></div>
			</div>
		{/each}
	</div>

<script lang="ts">
	...
	import { formV2, type SchemaType, type ModeType } from '../state/form-v2.svelte';
...
<Dialog.Root bind:open {onOpenChange}>
	<Dialog.Content class="flex h-[80vh] max-w-4xl flex-col">
		<Dialog.Header class="shrink-0">
			<Dialog.Title>Generated Code</Dialog.Title>
			<Dialog.Description>Copy the generated code to your project</Dialog.Description>
		</Dialog.Header>

		<!-- Controls -->
		<div class="flex shrink-0 items-center gap-4 py-2">
			<!-- Schema Selector -->
			<div class="flex items-center gap-2">
				<span class="text-sm text-muted-foreground">Schema:</span>
				<Select.Root type="single" value={schema} onValueChange={handleSchemaChange}>
		...
		<Tabs.Root bind:value={activeTab} class="flex min-h-0 flex-1 flex-col">
			<Tabs.List class="w-full shrink-0 justify-start">
				{#each tabs as tab}
			...
			{#each tabs as tab}
				<Tabs.Content value={tab.id} class="mt-2 min-h-0 flex-1">
								...
								{#if tab.id === 'install'}
									...
									>
										<Select.Trigger class="h-7 w-20 text-xs">
											{packageManagers.find((p) => p.value === packageManager)?.label ?? 'npm'}
										</Select.Trigger>
										<Select.Content>

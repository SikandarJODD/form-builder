...
<nav class="sticky top-0 z-50 border-b bg-background/70 backdrop-blur-xs">
	<div class="mx-auto max-w-5xl px-4 sm:px-4 lg:px-6 xl:px-0">
		<div class="flex h-16 items-center justify-between sm:h-14">
			<div class="flex w-full items-center">
				<a href="/" aria-label="home" class="flex shrink-0 items-center gap-1.5">
					<svg
						...
						/><path d="M10 10h11" /><path d="M10 3v18" /><path d="M9 3l-6 6" /><path
							d="M10 7l-7 7"
						/><path d="M10 12l-7 7" /><path d="M10 17l-4 4" /></svg
					>
					<span
				...
				<div class="hidden w-full items-center justify-between sm:ml-6 sm:flex">
					<div class="flex space-x-4">
						{#each navs as nav}
							<a href={nav.url}>
								<span class="text-muted-foreground transition-all duration-150 hover:text-primary"
									>{nav.name}</span
								>
							</a>
						{/each}
			...
			<div class="space-y-1 px-2">
				...
				{#each navs as nav}
					<a
						...
						href={nav.url}
						class="block rounded-md px-3 py-2 text-base font-medium text-primary">{nav.name}</a
					>
				{/each}
				<a
					onclick={() => (isMobileMenu = false)}
					href="/playground"
					class="block rounded-md px-3 py-2 text-base font-medium text-primary">Playground</a
				>
			</div>
...
{#snippet ThemeToggle()}
	<Button
		onclick={toggleMode}
		variant="outline"
		class="size-9 rounded-full [&_svg]:size-5"
		size="icon"
	>
		<!-- Sun Icon -->
...
{#snippet socials()}
	<div class="flex items-center space-x-1.5 md:space-x-1">
		...
		<Button
			size="icon"
			class="size-9 rounded-full [&_svg]:size-4"
			variant="outline"

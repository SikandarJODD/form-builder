<script lang="ts">
	...
	let triggerContent = $derived(select_examples.find((f) => f.value === value)?.label ?? 'Svelte');
	...
	const selectedValue = $derived(
		frameworks.find((f) => f.value === combovalue)?.label ?? 'Select a framework...'
	);
...
{#if form_generator.selected_inputs.length === 0}
	<div class="mt-4 flex size-full flex-col items-center justify-center">
...
{:else}
	...
	>
		<div class="mx-auto mt-4 flex max-w-md flex-col space-y-2 p-2">
			<div class="space-y-1">
				{#each form_generator.selected_inputs as comp (comp.id)}
					<div animate:flip={{ duration: 300 }}>
						...
						{#if comp.category === 'checkbox' && comp.type === 'boolean'}
							<div class="flex flex-row items-start space-y-0 space-x-3 rounded-md border p-4">
							...
							<div class="flex flex-col">
								<div class="grid gap-2">
									...
									<Popover.Root>
										...
										>
											{dvalue ? df.format(dvalue.toDate(getLocalTimeZone())) : 'Pick a date'}
								...
								<div class="flex flex-col">
									{#each Array.from(files) as file, i (file.name)}
										<div
											in:slide
											out:fly={{ x: 20 }}
											class="flex place-items-center justify-between gap-0.5 rounded-lg p-2 transition-all duration-200 hover:bg-accent dark:hover:bg-accent/60"
										>
						...
						{#if comp.type === 'tags-input'}
							<div>
								...
								<TagsInput
									...
									id={comp.named_id}
								/>
								<input type="hidden" name={comp.named_id} bind:value={tagsvalue} />
								<p class="text-xs text-muted-foreground">
									{comp.description}
								</p>
							</div>
						...
						{#if comp.type === 'combobox'}
							<div>
								...
								<div>
												...
												<Command.List class="w-full">
													...
													<Command.Group>
														{#each frameworks as framework}
															...
															>
																<Check
																	class={cn(combovalue !== framework.value && 'text-transparent')}
																/>
																{framework.label}
															</Command.Item>
														{/each}
									...
									<input type="hidden" name={comp.named_id} value={combovalue} />
								</div>
								<p class="text-xs text-muted-foreground">
									{comp.description}
								</p>
						...
						{#if comp.type === 'slider'}
							<div class="space-y-3">
								<Label class="text-sm font-medium" for="pricing">{comp.label} : {slidervalue}</Label
								>
								<Slider
									type="single"
									max={comp.max || 100}
									...
									step={1}
								/>
								<input type="hidden" name="slider" bind:value={slidervalue} id="slider" />
								<p class="text-sm text-muted-foreground">
									{comp.description}
								</p>
							</div>

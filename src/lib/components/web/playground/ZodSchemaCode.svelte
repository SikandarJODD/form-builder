<script>
  ...
  let triggerContent = $derived(
    all_schema.find((f) => f.value === schemavalue)?.label ?? "Schema"
  );
  let updateAdapter = () => {
...
>
  <div class="sticky top-2 gap-1.5 flex justify-end left-0">
    <div>
      <Select.Root
        name="schema"
        type="single"
      ...
      >
        <Select.Trigger class="w-[110px] focus:ring-0 h-8 text-xs">
          {triggerContent}
        </Select.Trigger>
        <Select.Content>
          <Select.Group>

...
<div class="relative rounded-lg border bg-zinc-900 dark:bg-transparent">
	<div class="absolute top-2 right-3 flex space-x-1.5">
		<div class="flex">
			<Button
				...
				class={[
					'z-50 rounded-s-lg rounded-r-none border-r-0',
					codeType === 'client' ? 'bg-secondary  text-primary' : 'text-muted-foreground'
				]}>Client</Button
			>
			<Button
				size="sm"
				...
				class={[
					'z-50 rounded-e-lg rounded-l-none',
					codeType === 'server' ? 'bg-secondary text-primary' : 'text-muted-foreground'
				]}>Server</Button
			>
		</div>
		<div class="flex flex-col gap-1.5">
			<Button class="z-50 rounded-lg" variant="outline" size="icon" onclick={handleCopy}>
	...
	<div class={['scrollbar max-h-[420px] overflow-scroll p-4 text-sm', blurbg && 'blur-xs']}>
		{#if codeType === 'client'}
			<div>
				{@html htmlClientCode}
			</div>

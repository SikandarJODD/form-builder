<script lang="ts">
	...
	let delete_selected_input = (id: string | undefined, type: string, name: string) => {
		if (id) {
...
<div
	class="col-span-1 w-full space-y-2 sm:col-span-2 sm:p-2"
	use:dragHandleZone={{
...
>
	{#each form_generator.selected_inputs as item (item.id)}
		<div
			...
			in:fly={{ y: 20 }}
			class="handle flex w-full items-center gap-1"
		>
			<button use:dragHandle aria-label="drag-handle for {item.id}" class="handle">
				<GripVertical strokeWidth={1.4} />
			</button>
			<div class="flex w-full items-center justify-between rounded-xl border px-2 py-2 sm:w-80">
				<span class="pl-1 text-sm capitalize">{item.name}</span>
				<div>
	...
	{:else}
		<span class="text-muted-foreground"> Add new Inputs for Creating Forms </span>
	{/each}
</div>

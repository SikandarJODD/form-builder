...
<section id="features">
	<div class="px-4 md:container md:px-0">
		<div class="mx-auto max-w-6xl">
			<div class="mx-auto my-0 grid h-full items-center gap-6 md:my-12 md:gap-10 lg:grid-cols-2">
				<div
          ...
          transition-duration: {$currentIndex === index ? `${collapseDelay}ms` : '0s'};"
				...
				<div class="h-[380px] md:min-h-[200px] {ltr && 'lg:order-1'}">
					{#if data[$currentIndex]?.image}
						<img
							...
							alt="feature"
							class="aspect-auto size-full rounded-xl border border-neutral-300/50 object-cover object-left p-1 shadow-lg"
						/>
				...
				<div
					class="relative mask-[linear-gradient(90deg,transparent,black_20%,white_80%,transparent)] pb-0.5 [-webkit-mask-image:linear-gradient(90deg,transparent,black_20%,white_80%,transparent)] md:hidden"
				>
				...
				>
					{#each data as item, index}
						...
						<li
							class="card_code relative mr-8 grid h-full w-full shrink-0 items-start justify-center pl-4 last:mr-0"

...
<svelte:head>
	<title>Password Input - Form Builder</title>
	<meta
		name="description"
		content="This is a password input component that includes a show/hide password feature."
	/>
	<meta
		name="keywords"
		content="password, password input, password-input compoennt, svelte, sveltekit, form-builder, location input, country, state"
	/>

	<meta property="og:title" content="Password Input - Form Builder" />
	<meta
	...
	<meta name="twitter:description" content="Build forms with ease using Svelte Form Builder." />
	<meta
		name="twitter:image"
		content="https://i.pinimg.com/736x/0d/22/63/0d2263fb3a30077c8e8f0def7187095f.jpg"
	/>
...
{@render npmcode('jsrepo add github/sikandarjodd/form-builder/comps/PasswordInput', handleCopy)}
{@render CodeSnippet('PasswordInput.svelte', rawcode)}
<h3 class="mt-6 text-xl font-semibold">Preview</h3>
<div
	class="mt-2 flex min-h-64 w-full items-center justify-center border border-dashed border-muted-foreground p-4"
>
...
{#snippet CodeSnippet(filename: string, code: string)}
	<div class="mt-4">
		<div
			class="ml-1 flex w-fit items-center gap-1.5 rounded-t-lg border border-b-0 bg-accent/80 p-2"
		>
			{#if filename.split('.').pop() === 'svelte'}
				<svg
					viewBox="0 0 256 308"
					width="18"
					height="18"
		...
		<div class="scrollbar relative w-full max-w-2xl overflow-hidden">
			<CopyCode {code} lang="svelte" class="h-[400px]" />
		</div>
	</div>
{/snippet}

{#snippet npmcode(text: string, handleCopy: (text: string) => void)}
	<div
		class="mt-2 flex w-fit min-w-64 items-center justify-between gap-x-5 rounded-lg bg-secondary py-1 pr-2 pl-4"
	>
